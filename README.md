...
```rust
use embedded\_graphics::{mono\_font::{MonoTextStyle, ascii::FONT\_6X10}, pixelcolor::BinaryColor};
use linux\_embedded\_hal::I2cdev;
use log::{error, info};
...
#[tokio::main]
async fn main() {
    let i2c\_interface = "/dev/i2c-1".to\_string();
    let i2c = I2cdev::new(i2c\_interface).expect("Failed to open I2C! Please enable I2C and connect the screen if you did not");
    let interface = I2CDisplayInterface::new(i2c);
    let mut display = Ssd1306::new(
        interface,
        DisplaySize128x64,
        DisplayRotation::Rotate0
    ).into\_buffered\_graphics\_mode();
    ...
    let app\_state = AppState { counter: 0 };
    ...
    let buttons = [(17, "fourth button"), (22, "third button"), (23, "second button"), (27, "first button")]
        .iter()
        .map(|pin| ButtonTag { pin: gpio.get(pin.0).expect(&format!("Failed to get GPIO pin {} with tag {}", pin.0, pin.1)).into\_input\_pullup(), tag: pin.1 })
    ...
    Engine::new(
        Settings { 
            ...
            fps: 20,
            fonts: vec![ Font { font: MonoTextStyle::new(&FONT\_6X10, BinaryColor::On), tag: "default" } ]
        },
        buttons,
        app\_state
    ).await.start\_rendering().await;
}
